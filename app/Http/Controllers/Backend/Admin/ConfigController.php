...
class ConfigController extends Controller
{
    ...
    public function getGeneralSettings()
    {
        ...
        $sections = Configs::where('key', '=', 'layout_' . $type)->first();
        $footer_data = Configs::where('key', '=', 'footer_data')->first();
    ...
    public function saveGeneralSettings(Request $request)
    {
        if (($request->get('mail_provider') == 'sendgrid') && ($request->get('list_selection') == 2)) {
            ...
            try {
                ...
                $sendgrid_list_id = Configs::where('sendgrid_list_id')->first();
        ...
        foreach ($requests->all() as $key => $value) {
            if ($key != '_token') {
                ...
                $config = Configs::firstOrCreate(['key' => $key]);
                $config->value = $value;
                $config->save();
    ...
    public function getSendGridLists(Request $request)
    {
        ...
        $sendgrid = Configs::firstOrCreate(['key' => 'sendgrid_api_key']);
