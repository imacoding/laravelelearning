...
@section('content')
    ...
    @foreach($threads as $item)
        @if($item->messages->count() > 0)
            @if($item->messages->last()->user->id!==auth()->user()->id)
            <a class="@if($item->userUnreadMessagesCount(auth()->user()->id) > 0) unread @endif" href="{{ route('admin.messages').'?thread='.$item->id }}">
                <div data-thread="{{ $item->id }}" class="chat_list @if(($thread != "") && ($thread->id == $item->id)) active_chat @endif">
                     
                    <div class="chat_people">
                        <div class="chat_ib">
                           
                            <h5>{{ $item->messages->last()->user->first_name }} <span class="chat_date">{{ $item->messages->last()->created_at->diffForHumans() }}</span>
                            ...
                            <p>{{ str_limit($item->messages->last()->body, 35) }}</p>
...
@endif
    ...
    <div class="chat_people box-header">
        <div class="chat_img float-left">
            <img src="{{ $thread->participants->where('id', '!=', auth()->user()->id)->first()->user->image }}"
                alt="{{ $thread->participants->where('id', '!=', auth()->user()->id)->first()->user->name }}" height="35px">
        </div>
        ...
        <div class="mesgs">
                            ...
                            <div class="outgoing_msg">
                                <div class="sent_msg">
                                    ...
                                    <span class="time_date text-right">
                                        {{ \Carbon\Carbon::parse($message->created_at)->format('h:i A | M d Y') }}
                                    </span>
                                </div>
                            </div>
                        @else
            ...
            <div class="type_msg">
                <div class="input_msg_write">
                    <textarea type="text" name="message" class="write_msg" placeholder="Type a message"></textarea>
